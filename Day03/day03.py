...
def is_symbol(x: int, y: int, my_input: [str]) -> bool:
    my_char = my_input[y][x]
    return my_char != "." and not str.isdigit(my_char)


def is_symbol_adjacent(x: int, y: int, my_input: [str]) -> bool:
    symbol_above = is_within_bounds(x, y - 1, my_input) and is_symbol(x, y - 1, my_input)
    symbol_below = is_within_bounds(x, y + 1, my_input) and is_symbol(x, y + 1, my_input)
    symbol_left = is_within_bounds(x - 1, y, my_input) and is_symbol(x - 1, y, my_input)
    symbol_right = is_within_bounds(x + 1, y, my_input) and is_symbol(x + 1, y, my_input)
    symbol_top_left = is_within_bounds(x - 1, y - 1, my_input) and is_symbol(x - 1, y - 1, my_input)
    symbol_top_right = is_within_bounds(x + 1, y - 1, my_input) and is_symbol(x + 1, y - 1, my_input)
    symbol_bottom_left = is_within_bounds(x - 1, y + 1, my_input) and is_symbol(x - 1, y + 1, my_input)
    symbol_bottom_right = is_within_bounds(x + 1, y + 1, my_input) and is_symbol(x + 1, y + 1, my_input)

    return (symbol_above or
            symbol_below or
            symbol_left or
            symbol_right or
            symbol_top_left or
            symbol_top_right or
            symbol_bottom_left or
            symbol_bottom_right)
...
def get_adjacent_numbers_diff_row(x: int, row: str) -> [str]:
...
def get_adjacent_numbers(x: int, y: int, my_input: [str]) -> [str]:
    max_y_index = len(my_input) - 1
    max_x_index = len(my_input[0]) - 1
    my_adjacent_numbers = []
    ...
    if y > 0:  # check row above
        my_adjacent_numbers += get_adjacent_numbers_diff_row(x, my_input[y - 1])
    ...
    for y, row in enumerate(my_input):
        for x, value in enumerate(row):
            if value != "." and not str.isdigit(value):  # i.e this is a SYMBOL
                my_numbers = get_adjacent_numbers(x, y, my_input)
                if len(my_numbers) == 2:
